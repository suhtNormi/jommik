...
<!-- popup kui vajutad kehaosa peale -->
    <div v-if="showPopup" class="popup">
      <div class="popup-content">
        <h3 class="popup-title">Exercises for <span>{{ currentCategory }}</span></h3>
        <ul>
    ...
    <!-- Slider millega saab toggleda kas tahad et annaks harjutusi v mitte -->
    <div class="toggle-container">
      <label class="toggle-label">Kas soovitan harjutusi?</label>
      <label class="switch">
        <input type="checkbox" v-model="showExercisesOnClick">
...
const {fetchExercises} = calendarStore();


export default {
  name: 'HumanSilhouette',
  // emits: ['click'],
  data() {
    return {
      ...
      // vaja paremini korda seada mis kehaosa mis harjutusi soovitab.
    },
    };
  },
  props: {
  ...
  methods: {
    part_clicked(event) {
      const partId = event.target.getAttribute('id');
      if (!partId || this.disabled) return;

      if (this.multiple) {
        // Handle array-based selection
        if (this.selection.includes(partId)) {
          this.selection = this.selection.filter((id) => id !== partId); // Remove if already selected
        } else {
          this.selection.push(partId); // Add new selection
        }
      } else {
        // Handle single selection
        this.selection = partId === this.selection ? '' : partId; // Toggle selection
      }

      // Emit the updated selection
      this.$emit('update:selection', this.selection);

      // Fetch exercises if the toggle is enabled
      if (this.showExercisesOnClick) {
        const category = this.getCategoryByPartId(partId);
        if (category) {
          this.fetchExercisesForCategory(category);
        }
      }
    },
    getCategoryByPartId(partId) {
      for (const [category, parts] of Object.entries(this.bodyPartCategories)) {
    ...
    async fetchExercisesForCategory(category) {
      try {
        this.currentCategory = category;
        this.exercises = await fetchExercises(category);
        this.showPopup = true;
      } catch (error) {
    ...
    toggleExercisePopup() {
      this.showExercisesOnClick = !this.showExercisesOnClick;
    },
    getStyle(partName) {
      const category = this.getCategoryByPartId(partName); // Map part to category
      const color = this.categoryColors[category] || this.bodyColour; // Fallback color
      return { fill: color }; // This ensures the path uses the correct fill color
    },
        // getStyle(partName) {
    //   const category = this.getCategoryByPartId(partName); // Map body part to category
    //   const color = this.$props.categoryColors[category] || this.bodyColour; // Get category color or default to body color

    //   const styles = {
    //     fill: color, // Apply the category color
    //   };

    //   // Highlight the selection
    //   if ((this.multiple && this.selection.includes(partName)) || this.selection === partName) {
    //     styles.fill = this.selectionColour; // Override with selection color
    //   }
    //   return styles;
    // },
    //   },
